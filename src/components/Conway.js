...
class Conway extends Component {
  ...
  drawBoard() {
    ...
    ctx.fillRect(
      25,
      25,
      this.state.canvasSize.width - 50,
    ...
    ctx.fillRect(
      ...
      5,
      this.state.canvasSize.width - 50,
      this.state.canvasSize.height - 10
    );
  }

  computeBuffer(buffer) {
    var nextBoard = [];
    for (var i = 0; i < buffer.length; i++) nextBoard.push(buffer[i].slice(0));
    let x = buffer.length,
      y = buffer[0].length;
  ...
  draw(caller) {
    setTimeout(
      function () {
        ...
        if (!this.state.shouldDraw) return;
        ...
        for (var i = 0; i < board.length; i++) {
          for (var j = 0; j < board[i].length; j++) {
            ...
            } else ctx.fillStyle = colorMap[colorInd].primary;
            ...
            for (var m = i - 1; m <= i + 1; m++) {
              for (var n = j - 1; n <= j + 1; n++) {
                ...
                if (this.props.confined) {
                  if (m >= board.length) row = 0;
                  else if (m < 0) row = board.length - 1;

                  if (n >= board[0].length) col = 0;
                  else if (n < 0) col = board[0].length - 1;
                }
                if (
                  (this.props.confined ||
                    (0 <= row &&
  ...
  handleCanvasClick(e) {
    ...
    if (
      j >= 0 &&
      j < this.state.board[0].length &&
      i >= 0 &&
      i < this.state.board.length
    ) {
  ...
  handleChange(event) {
    let speed =
      this.state.timeoutMax -
      (event.target.value * (this.state.timeoutMax - this.state.timeoutMin)) /
        100;
    this.setState({ speed });
  }
  setPlayer() {
